...
int extract_hextag(char *buffer, int size, char *dst) {
    int i = 0;
    char c = buffer[i];
    while (i < size && c != 0x00 && c != '\n') {
        // vérifie que c'est un caractère hexa.
...
int build_tag(char *name, char *date, char *hashmac, char *dst) {
    if (strlen(name) != 32 || strlen(date) != 32 || strlen(hashmac) != 32) {
        printf("error bnuild tag !\n");
        fflush(stdout);
        exit(-1);
    }
    memcpy(dst, name, 32);
    memcpy(&dst[32], date, 32);
    memcpy(&dst[64], hashmac, 32);
    return 96;
}
...
int authenticate(char *tag) {
    ...
    // tag = [ name(32) || date(32) || H(Key||name||date)(32) ] en hexa
    // Construction des blocs à hasher
    // str = "LeCTF2AMSI_est_Vraiment_Trop8bien_OnSamuseCommeDesFousTropBien__"
    char *K = "4c6543544632414d53495f6573745f567261696d656e745f54726f70386269656e5f4f6e53616d757365436f6d6d65446573466f757354726f704269656e5f5f";
    uint8_t blocks[97];
    bzero(blocks, 97);
    int blocks_size = build_blocks(K, tag, &tag[32], blocks);
    // printf("blocks : ");
    // print_bytes(blocks, blocks_size);
    // printf("tag : %s\n", tag);

    // Calcul du hash H(Key||name||date)
    struct md5_digest dgst;
    md5(blocks, &dgst, NULL);
    char hash_hex[33];
    // print_hash(dgst.bytes);
    hash_hex[32] = 0;
    bytes_to_hex(dgst.bytes, 16, hash_hex);
    // printf("hash_hex %s\n", hash_hex);
    // printf("tag %s\n", &tag[64]);
    // Compare le hash avec le mac
...
int main(int argc, char **argv, char **envp[]){
    ...
    char rfid_tag[97];
    bzero(rfid_tag, 97);
    // le tag par défaut de : jacques - 1986-03-03 - H(K||name||expiry)
    build_tag("6a616371756573000000000000000000", "313938362d30332d3033000000000000", "3a2e64d7a374bf564c1876dd3bd8fafe", rfid_tag);
    ...
    print_terminal("\e[33m[INFO] : entrez \"help\" pour obtenir de l'aide sur les commandes disponibles.\e[0m\n\n");
    ...
    while (running) {
        ...
        // switch for action
        if (!strncasecmp(buffer, "help", 4)) {
            print_terminal("Voici la liste des commandes disponibles : \n");
            print_terminal("\tread : Lit les données de la carte RFID\n");
            print_terminal("\twrite <hex_value> : Écrit le tag <hex_value> sur la carte RFID. Le tag attendu est au format hexadécimal.\n");
            print_terminal("\tscan : Passe la carte sur le lecteur RFID.\n\n");
        }
        else if (!strncasecmp(buffer, "read", 4)) {
            print_terminal("Démarrage du programmeur RFID...[OK]\n");
            print_terminal("Vérication des périphériques radio-fréquences...[OK]\n");
            print_terminal("Détection de la carte...[OK]\n");
            print_terminal("Lecture des données...[OK]\n");
            print_terminal("résultat (hexa) -> ");
            printf("%s", rfid_tag);
            fflush(stdout);
            print_terminal("\n\n");
        }
        else if (!strncasecmp(buffer, "write", 5)) {
            // vérification de la validité des données fournies :
            char tag[97];
            bzero(tag, 97);
            int r = extract_hextag(&buffer[6], 96, tag);
            if (r == -1) {
                print_terminal("Erreur ! les données ne sont pas au format hexadécimal.\n");
            }
            else if (strlen(tag) != 96) {
                print_terminal("Erreur ! la taille d'un tag doit être de 48 octets (96 caractères hexadécimaux) !");
            }
            else {
                print_terminal("Démarrage du programmeur RFID...[OK]\n");
                print_terminal("Vérication des périphériques radio-fréquences...[OK]\n");
                print_terminal("Détection de la carte...[OK]\n");
                print_terminal("Écriture des données...[OK]\n");
                memcpy(rfid_tag, tag, 96);
            }
        }
        else if (!strncasecmp(buffer, "scan", 4)) {
            // Scan la carte sur le lecteur RFID
            print_terminal("\e[34mVous passez la carte sur le lecteur RFID.\n");
            print_terminal("Une lumière orange clignote");
            usleep(500000);
            print_terminal(".");
            usleep(500000);
            print_terminal(".");
            ...
            fflush(stdout);
            // Vérification du tag.
            int r = authenticate(rfid_tag);
            if (r) {
                print_terminal("La LED passe au \e[33mvert !\e[0m");
                usleep(1000000);
                print_terminal("\e[34m Un bip sonore se fait entendre et la porte d'entrée s'ouvre !\n");
                print_terminal("Le capitaine Soap ordonne à Silver et Trivet de passer devant et de sécuriser le hall d'entrée.\n\n");
                usleep(2000000);
                print_terminal("Après quelques secondes, Trivet annonce : \e[0m\n\n");
                print_terminal("- Sécurisation de la pièce effectuée.");
                usleep(100000);
                print_terminal(" Aucun signe de menace potentiel détecté.");
                usleep(100000);
                print_terminal(" RAS.\n\n");
                usleep(1000000);
                print_terminal("\e[34mL'équipe entre dans le hall. Vous trouvez un papier à terre sur lequel il est écrit :\e[0m AMSI{BuG_D4ns_L1mpl3ment4t&0n!!!}\n");
                fgets(buffer, 2, stdin);

                return 0;
            }
            else {
                print_terminal("Zut ! La led est \e[31mrouge.\e[0m\n");
            }
        }
    }
